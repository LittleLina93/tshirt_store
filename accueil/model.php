...
function getNewestArticles() {
    ...
    $query = "SELECT id, title, creator, creation_date, price, image FROM articles ORDER BY creation_date DESC LIMIT 3 ";
...
function getRandomArticles() {
    global $db_default_connection;
    $query = "SELECT id, title, creator, creation_date, price, image FROM articles ORDER BY RAND() LIMIT 3 ";
    $stmt = $db_default_connection->prepare($query);
    $stmt->execute();
    $count = $stmt->rowCount();
